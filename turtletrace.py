...
class Turtletrace():
    def __init__(self, screen):
        ...
        self.velocity = 10
        self.deltaangle = myutils.degrees(1)
    ...
    def step(self):
        ...
        print(self.position)
        pygame.draw.line(self.canvas, (int(127*(numpy.cos(0.2*self.colorangle)+1)), int(127*(numpy.sin(0.5*self.colorangle)+1)), numpy.abs(int(127*(numpy.cos(0.9*self.colorangle)+1)))), self.lastposition, self.position, self.linesize)
        if not self.freeroam:
            self.screen.blit(self.canvas, (-self.position[0] + self.screen.get_width() / 2 + self.velocity*numpy.cos(self.angle), -self.position[1] + self.screen.get_height() / 2 - self.velocity*numpy.sin(self.angle)))
        else:
            self.screen.blit(self.canvas, self.cameraposition)
        
        self.lastposition = self.position
        self.angle += self.deltaangle
        self.deltaangle += numpy.pi / 4320
