...
class Trappedknight:
    ...
    def step(self):
        ...
        for i in range(0, len(self.moves)):
            ...
            if self.position[0] + self.moves[i][0] < 0 or self.position[1] + self.moves[i][1] < 0 or self.position[0] + \
                    self.moves[i][0] >= len(self.squares) or self.position[1] + self.moves[i][1] >= len(
                self.squares[0]):
                self.reinit()
                return

            if not self.squares[self.position[0] + self.moves[i][0]][self.position[1] + self.moves[i][1]][1]:
                if self.squares[self.position[0] + self.moves[i][0]][self.position[1] + self.moves[i][1]][
                    0] < lowestnumber or lowestnumber == 0:
                    nextposition = (self.position[0] + self.moves[i][0],
                                    self.position[1] + self.moves[i][1])
                    lowestnumber = \
                        self.squares[self.position[0] + self.moves[i][0]][self.position[1] + self.moves[i][1]][0]
        ...
        pygame.draw.line(self.canvas,
                         (int(127 * (numpy.cos(0.2 * self.colorangle) + 1)),
                          int(127 * (numpy.sin(0.5 * self.colorangle) + 1)),
                          numpy.abs(int(127 * (numpy.cos(0.9 * self.colorangle) + 1)))),
